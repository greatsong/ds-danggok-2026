...
성적 = max(내기록.items(), key=lambda x: (x[1]["안내 인원"] <= 정원, x[1]["찾아낸 환자"], -x[1]["안내 인원"]))
좋은모델, 값 = 성적
...
if 값["안내 인원"] <= 정원:
    ...
    if 이전 is None or (값["찾아낸 환자"], -값["안내 인원"]) > (이전["찾아낸 환자"], -이전["안내 인원"]):
        ...
        기록파일.write_text(json.dumps(st.session_state["최고기록"], ensure_ascii=False), encoding="utf-8")
...
if 최고:
    ...
    st.caption("이 기록은 모델 소개 페이지에 자동으로 표시됩니다. 새로고침해도 이 브라우저에 남습니다.")
else:
...
def 올리기(보낼것):
    ...
    읽기.open("GET", 저장주소 + "?select=class_id,nickname,found,sent,within_quota", False)
...
def 순위세기(줄들, 반, 팀명):
    ...
    찾아낸 환자가 많을수록, 같으면 안내 인원이 적을수록 앞선다."""
    최고 = {}
    for r in 줄들:
        if not r.get("within_quota"):
            continue
        열쇠 = (r["class_id"], r["nickname"])
        최고[열쇠] = max(최고.get(열쇠, (0, -정원)), (int(r["found"]), -int(r["sent"])))
    내점수 = 최고.get((반, 팀명))
    if 내점수 is None:
        return None
...
if 보낼까 and not 내팀명.strip():
    ...
    try:
        ...
        if 자리 is None:
            ...
            st.success(f"### 찾아낸 환자 {자리['내점수'][0]}명 · 안내 {-자리['내점수'][1]:,}명!\n"
