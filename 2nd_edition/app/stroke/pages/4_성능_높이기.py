...
빈값이름 = {"그대로 둔다": "중앙값으로 채운다", "지운다": "지운다"}   # 기록에는 왼쪽 이름으로 저장한다
...
st.info("고른 지표가 가장 높아지는 설정을 찾습니다. 지표가 같으면 찾아낸 환자가 많은 쪽, 그다음은 안내 인원이 "
        "적은 쪽이 앞섭니다. 마지막 목표는 안내 인원이 상담 정원 500명 이하인 채로 찾아낸 환자를 가장 많이 "
        "만드는 것입니다. 지표를 높일 때 찾아낸 환자는 어떻게 되는지 함께 봅니다. 테스트 데이터로 채점합니다.")
st.caption("비교에 쓰는 기본 설정은 11차시에서 사용한 것과 같습니다. 가중치 그대로 · 질문 3번 · 기준값 0.50. "
           "챌린지에서는 두 모델 모두 양쪽의 가중치를 같게 맞춰 학습하고, 기준값 0.60에서 시작합니다. "
...
def 순서(값, 목표):
    """목표에 따라 기록을 줄 세우는 열쇠. 클수록 앞선다."""
    if 목표 == "정원":
        return (값["안내 인원"] <= 정원, 값["찾아낸 환자"], -값["안내 인원"])
    점수 = 값[목표]
    return (점수 is not None, 점수 or 0, 값["찾아낸 환자"], -값["안내 인원"])
...
if 내목표 != "정원" or 값["안내 인원"] <= 정원:
    이전 = 최고들.get(내목표)
    if 이전 is None or 순서(값, 내목표) > 순서(이전, 내목표):
...
def 올리기(보낼것):
    ...
    읽기.open("GET", 저장주소 + "?select=class_id,nickname,goal,found,sent,within_quota,accuracy,recall,precision,f1",
             False)
...
def 순위세기(줄들, 반, 팀명, 목표):
    ...
    for r in 줄들:
        ...
        if 열쇠 not in 최고 or 순서(그값, 목표) > 순서(최고[열쇠], 목표):
            최고[열쇠] = 그값
    내것 = 최고.get((반, 팀명))
    if 내것 is None:
        return None
    내순서 = 순서(내것, 목표)
    우리반 = [v for (c, _), v in 최고.items() if c == 반]
    return {
        "내것": 내것,
        "전체인원": len(최고), "전체등수": sum(1 for v in 최고.values() if 순서(v, 목표) > 내순서) + 1,
        "반인원": len(우리반), "반등수": sum(1 for v in 우리반 if 순서(v, 목표) > 내순서) + 1,
    }


지문 = (내반, 내팀명.strip(), 내목표, 결측처리, tuple(입력열), 내깊이, 내기준)
