...
st.info("고른 지표가 가장 높아지는 설정을 찾습니다. 점수가 같으면 먼저 올린 팀이 앞섭니다. "
        "마지막 목표는 안내 인원이 상담 정원 500명 이하인 채로 찾아낸 환자를 가장 많이 만드는 것입니다. "
...
def 올리기(보낼것):
    ...
    읽기.open("GET", 저장주소 + "?select=class_id,nickname,goal,found,sent,within_quota,accuracy,recall,precision,f1,created_at",
             False)
...
def 순위세기(줄들, 반, 팀명, 목표):
    ...
    return {
        ...
        "반인원": len(우리반), "반등수": sum(1 for v in 우리반 if 앞선다(v)) + 1,
    }
