...
function initW_planecut(root, D) {
  ...
  function 그리기(깊이) {
    ...
    P.pts.forEach(p => dots.appendChild(el('circle', {cx: px(p[0]), cy: py(p[1]),
      r: p[2] ? 4.6 : 3.2, fill: 색[p[2]], 'fill-opacity': p[2] ? 0.95 : 0.5})));
    const acc = (P.acc || {})[깊이], moved = (P.moved || {})[깊이];
    info.textContent = '질문 ' + 깊이 + '개까지 · 칸 ' + 칸.length + '개'
      + (acc == null ? '' : ' · 테스트용 정확도 ' + acc.toFixed(3));
    바뀜.textContent = moved == null ? '' :
      (moved === 0 ? '선이 하나 늘었지만 판정이 바뀐 영화는 없습니다'
                   : '앞 단계와 비교하면 판정이 ' + moved + '편 바뀌었습니다');
    바뀜.setAttribute('fill', moved === 0 ? '#d64545' : '#6b7385');
