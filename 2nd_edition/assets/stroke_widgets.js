...
(function () {
      ...
      if (모드 === 'prob') {
        ...
        if (알림) {
          알림.textContent = '등고선은 0.1씩 그린 확률의 층입니다. 채점용 1,533명 가운데 가장 높은 확률이 '
            + 앞.pmax.toFixed(4) + '이고, 뇌졸중이라고 답한 사람은 ' + 앞.tp + '명입니다. 정확도는 '
            + 숫자(앞.acc) + '입니다.';
        }
      } else {
        var 나무 = D.d2.tree;
        ...
        if (알림) {
          알림.textContent = '질문 모델은 두 축을 직사각형 ' + 나무.rects.length
            ...
            + 숫자(나무.acc) + '이고 찾아낸 환자는 ' + 나무.tp + '명입니다.';
        }
      }
  ...
  // 무게를 맞추기 전과 뒤의 경계가 어디로 움직이는지 2D와 3D로 본다.
  function initW_boundaryShift(root, D) {
    ...
    function 그리기() {
      ...
      var 자취 = 산점도(묶음, '나이', '혈당');
      자취.unshift(등고선(D, 것.z, 색, 상태 === 'before' ? '무게 맞추기 전 확률' : '무게 맞춘 뒤 확률'));
      var 글상자 = [];
      ...
      if (알림) {
        알림.textContent = (상태 === 'before' ? '무게를 맞추기 전' : '무게를 맞춘 뒤')
